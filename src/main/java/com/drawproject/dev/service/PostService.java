...
public class PostService {
    ...
    public ResponsePagingDTO searchPosts(int page, int eachPage, String search, int categoryId){
        ...
        ResponsePagingDTO responsePagingDTO = new ResponsePagingDTO(HttpStatus.NOT_FOUND, "Posts not found",
                posts.getTotalElements(), page, posts.getTotalPages(), eachPage, posts.getContent());
    ...
    public List<String> saveImagesPost(List<MultipartFile> images, Authentication authentication) {
        String username = authentication.getName();
        User user = userRepository.findByUsername(username).orElseThrow();
        return images.stream()
                .map(image -> fileService.uploadFile(image, user.getUserId(), DrawProjectConstaints.IMAGE, "posts/content"))
                .toList();
    }
